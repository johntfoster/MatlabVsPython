...
# 
# The first claim is that MATLAB code is more readable to scientists and engineers than Python code that performs a similar task.  They use the following code examples to illustrate their point and even go as far to claim that there is a mistake in the Python operations.
# 
...
# They make the claim that the Python/NumPy solution doesn't have the correct dimensions to support the outer product operation.  This is because NumPy was created for element-wise array operations first and foremost.  It should be noted that this is a much cleaner syntax for the type of array computing that is typically done in data science, but nevertheless, their example can easily be corrected by simply adding a second set of brackets to explicitly add the second dimension to the row vector.  Initially it will have a shape (1, 3) instead of (3,).  This can be seen by looking at the array's shape attribute.
...
# Now you might say that the `@` operator is not how we would typically write matrix/tensor mathematics in handwriting or in typesetting a book or paper.  That is true, but that is the same for the `*` operator.  If you want to be extremely verbose, you could use Einstein indicial notation, i.e.
# 
# \begin{align}
...
# It's impossible to understand the argument here.  Both functions' documentation are equally clear in explanation. The same is true for all of the functions in NumPy and SciPy.
# 
# 
# ## Machine learning
# 
...
# 
# However, as Python has a much larger overall user base, there are many examples of complicated applications built using open source tools freely available, e.g. [New York City Taxi Explorer](https://anaconda.org/jbednar/nyc_taxi_panel/notebook).  In fact, we can build a simple user interface for curve-fitting in just a few lines of Python.
...
# 
# Finally, most of the rest of the arguments in their post make statements related to a lack of integrated features  in Python. I assume they are talking about the core Python language and this is very misleading in the context of Scientific computing because no one works in this space attempts to do so with pure Python, everyone uses the libraries discussed in this post and commonly, folks are simply installing the Anaconda Distribution in which all of these packages *are integrated* into one maintained system with compatible libraries, visualization tools, sand-boxed environments for exploration and prototyping, a choice of IDEs, and many more features.
# 
# This post can be acessed in its original format as a Jupyter notebook [here](https://github.com/johntfoster/MatlabVsPython/blob/master/MatlabVsPython.ipynb).  
# 
# *Of course, you will need a running installation of MATLAB and possibly have to change the file paths to the executable to get this those code cells with the MATLAB timings to work properly.*
